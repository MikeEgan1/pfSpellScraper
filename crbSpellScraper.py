...
def scrape_spell_page(soup):
    ...
    spell["school"], spell["levels"] = parse_school_and_levels(paragraphs[1])
    spell["action"] = parse_action(paragraphs[2])
    spell["components"] = parse_components(paragraphs[3])
...
def parse_components(paragraph):
    print paragraph.text.replace("Components", "").replace("Component:", "").split(",")
